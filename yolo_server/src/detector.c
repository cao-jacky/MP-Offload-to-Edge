...
void load_params() {
    options = read_data_cfg("data/paris.data");
    name_list = option_find_str(options, "names", "data/paris.names");
    names = get_labels_custom(name_list, &names_size); //get_labels(name_list);

    alphabet = load_alphabet();
    net = parse_network_cfg_custom("cfg/yolov3-tiny-paris.cfg", 1, 1); // set batch=1
    load_weights(&net, "yolov3-tiny-paris.weights");
...
struct result* detect()
{
    ...
    results.objects = (struct object*)malloc(sizeof(struct object)*results.num+256);
